...
def get_model_critic():
    ...
    model.compile(optimizer=tf.compat.v1.train.AdamOptimizer(learning_rate=lr), loss="mse")
    return model
...
class estimator:
    ...
    def train_actor(self, buffer_obs, buffer_logprob, A_k):
        with tf.GradientTape() as tape:
            ...
            for obs in buffer_obs:
                ...
                mean = self.actor(obs, training=True)
        ...
        self.optimizer_actor.apply_gradients(zip(grads, self.actor.trainable_weights))
    ...
    def train_critic(self, buffer_obs, buffer_rtgs):
        ...
        self.optimizer_critic.apply_gradients(zip(grads, self.critic.trainable_weights))
